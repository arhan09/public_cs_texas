...
# main idea here is to construct a history of leasing and subsequent leasing
# revenues (bonus, rentals (??) and royalties) at the parcel level, and instead
# of matching leases to parcels using GIS, use LeaseLandFile.csv which
# explicitly matches mineral file numbers to control numbers
...
# =============================================================================
# use recent shapefiles.  these are somewhat more recent than our most recent
# auction data entry, FYI
active <-
...
shapefile_lease_terms <-
  ...
  extract(Term,
          ...
          regex("(\\d+) years, (\\d+) months, (\\d+) days", ignore_case = T),
          convert = TRUE) %>%
...
tabular_lease <-
  ...
  mutate(Effective_Date = as_date(ymd_hm(Effective_Date)),
         ...
         Lease_Status_Update = as_date(ymd_hm(Lease_Status_Update))) %>%
