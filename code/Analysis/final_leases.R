...
# COME BACK HERE
# maybe just simplify by selecting the things from lease outcomes that actually
# get used here: bonus, royalty, leaserevenue, boe
...
# note that the "Effective_Date" in this dataset is rounded to the month, so
# when we merge this we may need to adjust when the first payment date is
peak_time_to_drill <-
  clean_lease_outcomes_monthly %>%
  filter(!PreEffectiveDate) %>%
  filter(BOE_total > 0) %>%
