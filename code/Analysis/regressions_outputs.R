...
setFixest_fml(..depvars = ~ c(DBOE, LeaseRevenue, SellerRevenue),
...
linear_leaserev_tbl <-
  c(as.list(linear_models_fe[lhs = 1]),
    ...
    list(linear_models_extra_fe[lhs=1])) %>%
  make_table(output_format = "df")

linear_dboe_tbl <-
  c(as.list(linear_models_fe[lhs = 2]),
    list(linear_models_dml[[2]]),
    list(linear_models_extra_fe[lhs=2])) %>%
  make_table(output_format = "df")

linear_output_tbl <-
  list(linear_leaserev_tbl, linear_dboe_tbl) %>%
...
poisson_leaserev_tbl <-
  c(as.list(poisson_models_fe[lhs = 1]),
    ...
    list(poisson_models_extra_fe[lhs=1])) %>%
  make_table(output_format = "df", stats = NA, stats_names = NA)

poisson_dboe_tbl <-
  c(as.list(poisson_models_fe[lhs = 2]),
    list(poisson_models_dml[[2]]),
    list(poisson_models_extra_fe[lhs=2])) %>%
  make_table(output_format = "df", stats = NA, stats_names = NA)

poisson_output_tbl <-
  list(poisson_leaserev_tbl, poisson_dboe_tbl) %>%
  regtable_stack(table_names = c("Lease Revenue", "Output"),
                 n_bottom = TRUE,
                 output_format = "latex")

# seller revenue main text table
linear_sellerrev_tbl <-
  c(as.list(linear_models_fe[lhs = 3]),
    list(linear_models_dml[[3]]),
    list(linear_models_extra_fe[lhs = 3])) %>%
  make_table(output_format = "df", stats = NA, stats_names = NA)

poisson_sellerrev_tbl <-
  c(as.list(poisson_models_fe[lhs = 3]),
    list(poisson_models_dml[[3]]),
    list(poisson_models_extra_fe[lhs = 3])) %>%
  make_table(output_format = "df", stats = NA, stats_names = NA)

sellerrev_tbl <-
  list(linear_sellerrev_tbl, poisson_sellerrev_tbl) %>%
  regtable_stack(table_names = c("Linear", "Poisson"),
                 n_bottom = TRUE,
                 output_format = "latex")
