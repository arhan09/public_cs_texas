...
#===============================================================================
# estimate EverLeased model
#===============================================================================
negotiation_ever_leased <-
  with(filter(regdata_everleased, Auction == 0), mean(EverLeased) * 100)

latex_number(negotiation_ever_leased,
             "negotiation_ever_leased",
             format = "f",
             big.mark = ",",
             digits = 2)
...
everleased_fe <-
  feols(EverLeased ~ Auction + bs(Acres, df = 4) | ..fe, regdata_everleased)

everleased_surfthick_fe <-
  feols(EverLeased ~ Auction + ..surface + ShaleThickness + bs(Acres, df = 4) |
          ..fe, regdata_everleased)

everleased_dml <-
  dml(EverLeased ~ Auction | Acres + CentLat + CentLong, regdata_everleased,
      "linear", n = dml_n, ml = "rf", workers = 8)

everleased_surfthick_dml <-
  paste("EverLeased",
        dml_fml(extra_controls, "ShaleThickness"),
        sep = " ~ ") %>%
  as.formula %>%
  dml(filter(regdata_everleased, !is.na(ShaleThickness)),
      "linear", n = dml_n, ml = "rf", workers = 8)
...
# if you want a (quick) update, revise dml_n in the texas_constants.R file to
# something like 11
#===============================================================================
parcel_models_dml <-
  dml_pa_outcomes %>%
...
#===============================================================================
everleased <-
  list(everleased_fe[fixef=1], everleased_fe[fixef=2],
       everleased_dml,
       everleased_surfthick_fe[fixef=1], everleased_surfthick_fe[fixef=2],
       everleased_surfthick_dml) %>%
  regtable(est = "Auction",
           est_names = c("Auction"),
           extra_rows = list("Estimate" = rep(c("G10", "G20", "DML"),
                                           2),
                             "Extra" = c("No", "No", "No",
                                         "Yes", "Yes", "Yes")),
           n_obs = TRUE,
...
#===============================================================================
# save parcel tables to disk
#===============================================================================
writeLines(everleased, file.path(tdir, "everleased.tex"))
