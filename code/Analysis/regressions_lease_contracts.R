...
# "robustness" tables: there are going to be 3 of these:
# logbonus
# royalty/term stack
# bonus (for appendix)
# write a function to do this
make_robustness_table <- function(lhs, output_format = "latex") {
  ms <-
    list(robust_lease_contract_models_fe[sample = 1, lhs = lhs, rhs = 2],
         robust_lease_contract_models_dml[[lhs]][[1]],
         robust_lease_contract_models_fe[sample = 1, lhs = lhs, rhs = 3],
         robust_lease_contract_models_dml[[lhs]][[2]],
         robust_lease_contract_models_fe[sample = 3, lhs = lhs, rhs = 1],
         robust_lease_contract_models_dml[[lhs]][[3]])
  ...
  tbl <-
    regtable(ms,
             ...
             stats = c("r.squared"),
             stats_names = c("$R^2$"),
             se_fun = list(vcov10, vcov, vcov10, vcov, vcov10, vcov),
             decimals = 2,           
             output_format = output_format)
  return(tbl)
}
...
# robustness tables
logbonus_robustness_tbl <- make_robustness_table(1)
bonus_robustness_tbl <- make_robustness_table(4)

#===============================================================================
# save tables to disk
#===============================================================================
writeLines(bonus_main_tbl, file.path(tdir, "bonus_regressions.tex"))
writeLines(bonus_robustness_tbl,
           file.path(tdir, "bonus_regressions_discussion.tex"))

writeLines(logbonus_main_tbl, file.path(tdir, "logbonus_regressions.tex"))
writeLines(logbonus_robustness_tbl,
